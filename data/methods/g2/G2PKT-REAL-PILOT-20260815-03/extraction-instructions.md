# Frozen G2 packet-03 corrected atomic extraction instructions

Packet ID: `G2PKT-REAL-PILOT-20260815-03`
...
`schemas/g2_atomic_extraction_row.schema.json`, the rules in
`config/g2_atomic_semantic_contract.json`, and the eight exact source-boundary
corrections in `config/g2_atomic_boundary_correction_packet03.json`.

Do not inspect any packet 01 or packet 02 extraction output, receipt,
difference, comparison or advisory report. Do not inspect any packet-03 output
belonging to the other extraction role. Procedural role separation does not
establish cognitive, institutional, specialist or human independence.
...
## Packet-03 boundary correction

The following eight critical fields have an exact required source-native value.
They clarify only occurrence boundaries and do not change any value, code,
meaning, quarantine disposition or source edition:

- BRA `locator_section_source`: `Dados de processos de violência doméstica e familiar contra a mulher`
- ZAF `clock_label_source`: `within 90 days from date of proper service of process`
- ZAF `locator_object_source`: `Table 20: Percentage Maintenance matters finalised within 90 days from date of proper service of process.`
- ZAF `measure_label_source`: `% Finalised Within 90 Days`
- USA-MN `clock_label_source`: `Fiscal Year`
- USA-MN `series_label_source`: `Family`
- AUS `matter_label_source`: `Applications for final orders`
- AUS `series_label_source`: `applications for final orders filed, finalised and pending, 2022–23 to 2024–25`

If the bound source does not support one of these exact values, fail the run;
do not infer, repair or substitute another boundary.
...
- headline: displayed total protective measures for the displayed 2026 snapshot
